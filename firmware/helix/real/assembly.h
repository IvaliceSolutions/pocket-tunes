...
#elif defined(HELIX_GENERIC_C)

// Portable C fallback (used only for the native host test — bit-exact with
// the rv32 `mulh` path since both keep the top 32 bits of the 64-bit product).
typedef long long Word64;
...
static __inline int CLZ(int x) {
	int n = 0;
	if (!x) return (int)(sizeof(int) * 8);
	while (!((unsigned)x & 0x80000000u)) { n++; x <<= 1; }
	return n;
}
static __inline Word64 MADD64(Word64 sum, int a, int b) {
	return sum + ((long long)a * (long long)b);
}
